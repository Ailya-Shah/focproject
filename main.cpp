...
void playLevel(const std::vector<Puzzle>& puzzles, const std::string& mode, int& points) {
 ...
 sf::Texture backgroundTexture;
    if (!backgroundTexture.loadFromFile("simon.png")) {
        std::cerr << "Error loading background image! Ensure 'simon.png' exists in the executable directory.\n";
        return;
    }
...
bool identifyMurderer(const std::vector<Suspect>& suspects, const Murderer& murderer) {
    ...
    while (attempts < maxAttempts) {
        std::cout << "Choose the murderer (Enter the number of the suspect): ";
        std::cin >> choice;

        if (std::cin.fail() || choice <= 0 || choice > suspects.size()) {
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            std::cout << "Invalid choice. Please enter a valid number.\n";
        } else if (suspects[choice - 1].name == murderer.name) {
            std::cout << "Correct! The murderer is " << murderer.name << "!\n";
            return true;
        } else {
...
int main() {
    ...
    std::vector<Puzzle> hardPuzzles = {
        {"eciveend", "evidence", "I confirm what really happened. What am I?"}
    };
    ...
    playLevel(selectedPuzzles, modeChoice, points);
